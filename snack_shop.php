...
$allowed_category = ['drinks', 'snacking', 'coffee', 'all'];
if (!in_array($category, $allowed_category)) {
    redirectWithError("Invalid category selected.", "snack_portal.php");
}
...
if ($category === 'all') {
    ...
    $query .= $isAdmin ? "" : " AND restricted = 0";
    $stmt = $pdo->prepare($query);
...
?>
    ...
    <div id="main-part">
        <h2>Snack Shop - <?= $category === 'all' ? 'All products' : ucfirst($category) ?></h2>
